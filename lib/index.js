...
function getConfig() {
    var _a, _b;
    return __awaiter(this, void 0, void 0, function* () {
        let { url, username, password, projectId, outDir = './src/assets/iconfont', language = 'js' } = (_a = require((0, utils_1.getRootFilePath)('./package.json')).autoIconfont) !== null && _a !== void 0 ? _a : {};
        if (!(0, utils_1.judgeIsVaildUrl)(url)) {
            if (!username || !password || !projectId)
                throw new Error('iconfont 地址异常，并且项目信息和用户信息异常');
            const iconfont = new IconFont_1.default({ username, password, projectId });
            yield iconfont.init();
